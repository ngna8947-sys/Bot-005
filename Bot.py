...
def _generate_khqr(uid, amount, note=""):
    ...
    # Backup KHQR Generator ធានាចេញ 100% មិន Error
    try:
...
def _generate_styled_khqr_image(qr_str, amount, merchant_name="KhmerSMM"):
    ...
    draw.rectangle([(0, 0), (card_w, 28)], fill="#00465c")

    font_aba, font_slogan, font_name, font_khqr_small, font_amt, font_dollar = None, None, None, None, None, None
    ...
    if not font_aba:
        font_aba = font_name = font_slogan = font_khqr_small = font_amt = font_dollar = ImageFont.load_default()
    ...
    draw.text((start_x + w_aba, 100), pay_text, fill="#00a3b8", font=font_aba)
    draw.text((card_w // 2, 175), "Scan. Pay. Done.", fill="#111111", font=font_slogan, anchor="mm")
...
def handle_callbacks(call):
    ...
    if data.startswith("dep:"):
        ...
        if not kb:
            ...
            bot.edit_message_text("⚡️ <b>សូមជ្រើសរើសប្រភេទបណ្តាញសង្គម៖</b>", chat_id=uid, message_id=call.message.message_id, reply_markup=kb)
...
def handle_messages(message):
    ...
    if uid == ADMIN_ID and step == "admin_game_title":
        ...
        waiting[uid] = {"step": "admin_game_zone", "gkey": gkey, "title": text}
        kb = InlineKeyboardMarkup([
    ...
    if uid == ADMIN_ID and isinstance(step, dict) and step.get("step") == "admin_item_price":
        ...
        if gkey not in games_db:
            games_db[gkey] = {
                "title": step["title"],
                "req_zone": step["req_zone"],
                "items": [],
            }

        games_db[gkey]["items"].append({
